			...
			<div class="row">
				<div class="col-lg-12">
					<div id="filter" class="card card-block" style="margin-bottom: 20px;">
						<h5>Filter</h5>
						<form action="<?=base_url()?>admin/filter_data" method="post" class="row">
							<div class="col-lg-3">
								<p>RW</p>
								<select name="rw" id="" class="form-control">
									<option value="0">Semua</option>
									<?php for ($i=1; $i <= count($rw); $i++){ ?>
										<option value="<?=$rw[$i-1]?>" <?php if($this->session->userdata('filter')) {if($this->session->userdata('rw') == $rw[$i-1]) echo 'selected';} ?>>RW <?=$rw[$i-1]?></option>
									<?php } ?>
								</select>
							</div>
							<div class="col-lg-3">
					...
					<?php if($this->session->userdata('filter')) { ?>
						<form action="<?= base_url() ?>admin/remove_filter">
							...
							<tbody>
								<?php $no = 1; for ($i=0; $i < count($warga); $i++) { ?>
									<tr>
										...
										<td>
											<a href="<?= base_url() ?>admin/edit_warga/<?=$warga[$i]['nik']?>">
...
<div class="modal fade" id="delete" tabindex="1" role="dialog" aria-labelledby="deleteModal" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<form action="<?= site_url('admin/delete_warga/') ?>" method="post">
